...
var (
	filter_VersionService_Apply_0 = &utilities.DoubleArray{Encoding: map[string]int{"product": 0, "operatorVersion": 1, "apply": 2}, Base: []int{1, 1, 2, 3, 0, 0, 0}, Check: []int{0, 1, 1, 1, 2, 3, 4}}
)
...
func request_VersionService_Apply_0(ctx context.Context, marshaler runtime.Marshaler, client VersionServiceClient, req *http.Request, pathParams map[string]string) (proto.Message, runtime.ServerMetadata, error) {
	...
	val, ok = pathParams["operatorVersion"]
	if !ok {
		return nil, metadata, status.Errorf(codes.InvalidArgument, "missing parameter %s", "operatorVersion")
	}

	protoReq.OperatorVersion, err = runtime.String(val)

	if err != nil {
		return nil, metadata, status.Errorf(codes.InvalidArgument, "type mismatch, parameter: %s, error: %v", "operatorVersion", err)
	}
...
func RegisterVersionServiceHandlerServer(ctx context.Context, mux *runtime.ServeMux, server VersionServiceServer) error {
	...
	mux.Handle("GET", pattern_VersionService_Apply_0, func(w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
		...
		rctx, err := runtime.AnnotateIncomingContext(ctx, mux, req, "/example.VersionService/Apply")
		if err != nil {
...
func RegisterVersionServiceHandlerClient(ctx context.Context, mux *runtime.ServeMux, client VersionServiceClient) error {
	...
	mux.Handle("GET", pattern_VersionService_Apply_0, func(w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
		...
		defer cancel()
		inboundMarshaler, outboundMarshaler := runtime.MarshalerForRequest(mux, req)
		rctx, err := runtime.AnnotateContext(ctx, mux, req, "/example.VersionService/Apply")
		if err != nil {
			runtime.HTTPError(ctx, mux, outboundMarshaler, w, req, err)
			return
		}
...
var (
	pattern_VersionService_Apply_0 = runtime.MustPattern(runtime.NewPattern(1, []int{2, 0, 2, 1, 2, 2, 1, 0, 4, 1, 5, 3, 1, 0, 4, 1, 5, 4, 1, 0, 4, 1, 5, 5}, []string{"api", "versions", "v1", "product", "operatorVersion", "apply"}, ""))
)
