...
All built-in PMM alert templates work with this view automatically. If you use custom templates, you can add `node_name` and `service_name` labels to make your alerts appear here too. To learn how, see [View alert status by node or service](https://docs.percona.com/percona-monitoring-and-management/3/alert/alert_status.html).

![Alert status page](https://docs.percona.com/percona-monitoring-and-management/3/images/Alert_status.png)
...
Since the Grafana PostgreSQL datasource does not support a read-only mode, removing the provisioned datasource is the most effective way to eliminate this risk. If you need it, you can [add it manually](https://docs.percona.com/percona-monitoring-and-management/3/admin/security/index.md#postgresql-datasource).

This update completes the security hardening that began in [PMM 3.7.0](https://docs.percona.com/percona-monitoring-and-management/3//3.7.0.html), which addressed a remote code execution vulnerability in the same datasource ([CVE-2026-25212](https://nvd.nist.gov/vuln/detail/CVE-2026-25212)).
...
![Disable collectors field in Additional options](https://docs.percona.com/percona-monitoring-and-management/3/images/Disable_Collectors.png)
...
For the full list of available collectors per database type, see [Connect MySQL databases to PMM](https://docs.percona.com/percona-monitoring-and-management/3/install-pmm/install-pmm-client/connect-database/mysql/mysql.html) and [Connect PostgreSQL databases to PMM](https://docs.percona.com/percona-monitoring-and-management/3/install-pmm/install-pmm-client/connect-database/postgresql.html).

### New panels on the PostgreSQL Top Queries dashboard

The [PostgreSQL Top Queries](https://docs.percona.com/percona-monitoring-and-management/3/reference/dashboards/dashboard-postgresql-topqueries.html) dashboard now includes four new panels to help you pinpoint queries that are putting the most pressure on your database:
...
![New panels on PostgreSQL Top Queries dashboard](https://docs.percona.com/percona-monitoring-and-management/3/images/pg-panels.png)
...
![Export RTA button](https://docs.percona.com/percona-monitoring-and-management/3/images/ExportRTA.png)

See [Export RTA data](https://docs.percona.com/percona-monitoring-and-management/3/use/qan/QAN-realtime-analytics.md#export-data) for step-by-step instructions and a reference of all exported fields.

### PMM Server UI upgrade removed

As announced in [PMM 3.7.0](https://docs.percona.com/percona-monitoring-and-management/3/release-notes/3.7.0.md#added-support-and-deprecations), PMM 3.9.0 removes the **Upgrade Now** button, UI-based upgrade functionality, and Watchtower from the PMM Server image.
...
* **[Docker upgrade](https://docs.percona.com/percona-monitoring-and-management/3/pmm-upgrade/upgrade_docker.html) (recommended)**: pull the latest image and restart the container
* [Podman upgrade](https://docs.percona.com/percona-monitoring-and-management/3/pmm-upgrade/upgrade_podman.html): same container workflow with rootless execution
* [Helm upgrade](https://docs.percona.com/percona-monitoring-and-management/3/pmm-upgrade/upgrade_helm.html): Kubernetes-native upgrade path
...
To get started, [install PMM Client](https://docs.percona.com/percona-monitoring-and-management/3/install-pmm/install-pmm-client/package_manager.html) on your Ubuntu 26.04 system.
...
- [PMM-14502](https://perconadev.atlassian.net/browse/PMM-14502): Added a dedicated topic for [running custom queries on MySQL](https://docs.percona.com/percona-monitoring-and-management/3/install-pmm/install-pmm-client/connect-database/mysql/custom_queries.html) so that you can collect any MySQL metric accessible via SQL and monitor it in PMM alongside your standard dashboards.
...
- [PMM-14895](https://perconadev.atlassian.net/browse/PMM-14895): Updated the **Replication Roles** section of the [Valkey/Redis Cluster Details](https://docs.percona.com/percona-monitoring-and-management/3/reference/dashboards/dashboard-valkey-redis-cluster-details.html) dashboard to use `Primary` and `Replica` instead of the outdated `master`/`slave` terminology.
...
- [PMM-14717](https://perconadev.atlassian.net/browse/PMM-14717): Fixed a documentation issue in the [MongoDB connection guide](https://docs.percona.com/percona-monitoring-and-management/3/install-pmm/install-pmm-client/connect-database/mongodb.html) where the `createRole` command was missing the `find` permission on `system.profile`, causing QAN to show no query data. If you set up MongoDB monitoring using the previous instructions, re-run the role creation with the corrected command.
- [PMM-15130](https://perconadev.atlassian.net/browse/PMM-15130): Fixed an issue where `pmm-admin inventory change agent` would save new database credentials without validating them. When rotating credentials from the command line, PMM now returns an error if the credentials are invalid. To bypass validation when the database is temporarily unreachable, use [--skip-connection-check](https://docs.percona.com/percona-monitoring-and-management/3/use/commands/pmm-admin/inventory.md#connection-and-authentication).
- [PMM-14921](https://perconadev.atlassian.net/browse/PMM-14921): Clarified the [Configure Nomad](https://docs.percona.com/percona-monitoring-and-management/3/reference/nomad.html) instructions. Nomad won't start if `PMM_ENABLE_NOMAD=1` is set without `PMM_PUBLIC_ADDRESS`. The prerequisites section and Docker examples now show that both variables are required together.
...
- [PMM-15165](https://perconadev.atlassian.net/browse/PMM-15165): The **Cluster** filter in the [MongoDB Router Summary](https://docs.percona.com/percona-monitoring-and-management/3/reference/dashboards/dashboard-mongodb-router-summary.html) dashboard was empty and returned no data. This is now fixed.
...
- [PMM-15155](https://perconadev.atlassian.net/browse/PMM-15155): After upgrading PMM, the [PostgreSQL Overview](https://docs.percona.com/percona-monitoring-and-management/3/reference/dashboards/dashboard-postgresql-instances-overview.html) dashboard showed a `Datasource not found` error. This is now fixed.
...
- [PMM-15076](https://perconadev.atlassian.net/browse/PMM-15076): The [NUMA Details dashboard](https://docs.percona.com/percona-monitoring-and-management/3/reference/dashboards/dashboard-numa-details.html) only showed NUMA Node 0 instead of all available nodes. You can now select individual nodes or view all nodes at once from the **Node** dropdown.
...
- [PMM-14885](https://perconadev.atlassian.net/browse/PMM-14885): The **Replicas** panel in the [PostgreSQL Replication Overview](https://docs.percona.com/percona-monitoring-and-management/3/reference/dashboards/dashboard-postgresql-replication.html) dashboard showed no data due to an invalid query. This is now fixed.
...
The upgrade completes successfully in the background. Refresh the page to confirm it finished. For future upgrades, see [PMM Server UI upgrade removed](https://docs.percona.com/percona-monitoring-and-management/3//#pmm-server-ui-upgrade-removed).
