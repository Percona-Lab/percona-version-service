...
```
If you're using an external PostgreSQL database, make sure to update your PMM Server configuration before upgrading to PMM 3.2.0. 
```
This is because Grafana 11.6 has a [regression issue](https://github.com/grafana/grafana/issues/102337) where the `GF_DATABASE_URL` environment variable is no longer sufficient for database configuration.  See [Migrate External PostgreSQL Configuration for PMM 3.2.0+](https://docs.percona.com/percona-monitoring-and-management/3/pmm-upgrade/external_postgres_pmm_upgrade.html) for step-by-step instructions.
...
Nomad is disabled by default. Even though it increases the overall Docker image size, it has no performance impact when not enabled. For more information, see [Nomad configuration](https://docs.percona.com/percona-monitoring-and-management/3/reference/nomad.html).
